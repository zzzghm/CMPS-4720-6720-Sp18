...
"""
load olivettifaces，divide into train_data,valid_data,test_data
function return train_data,valid_data,test_data and the corresponding label
"""
def load_data():
    ...
    #diving into three set and the sizes are following
...
#classificaion, the last layer, softmax
...
def evaluate_olivettifaces(learning_rate=0.05, n_epochs=200,
    ...
    #random number generator, used to initialize the parameters
    ...
    #x represent faces, as the input of layer0
    index = T.lscalar()
    x = T.matrix('x')  
    y = T.ivector('y')
    ...
    # the first layer: conv+maxpool
    # convolution：(57-5+1 , 47-5+1) = (53, 43)
    # maxpooling： (53/2, 43/2) = (26, 21)，ignore the boundary
    ...
    # the second layer: conv+maxpool input is the output of the last layer (batch_size, nkerns[0], 26, 21)
    # convolution：(26-5+1 , 21-5+1) = (22, 17)
    # maxpooling： (22/2, 17/2) = (11, 8)，ignore the boundary
    ...
    # HiddenLayer: fully connect，input size(batch_size, num_pixels)，making feature map of every sample to a one-dimension vector after layer0 and layer 1  
    #the output of last layer (batch_size, nkerns[1], 11, 8) convert to(batch_size, nkerns[1] * 11* 8),with flatten
    layer2_input = layer1.output.flatten(2)
    layer2 = HiddenLayer(
        rng,
        input=layer2_input,
        n_in=nkerns[1] * 11 * 8,
        n_out=2000,      #the number of neurons in the output of full connect layer
        activation=T.tanh
    )

    #classification
    layer3 = LogisticRegression(input=layer2.output, n_in=2000, n_out=40)   #n_in is the output of the fully-connect layer，n_out is 40 labels


    ###############
    # modify the algorithm: cost function, GSD
    ###############
    # cost function
    ...
    #train_model update parameter accrding to MSGD during the training
    ...
    ###############
    # train CNN, fing the parameters
    ###############
    print ('... training')
    #in LeNet5，batch_size=500,n_train_batches=50000/500=100，patience=10000
    #in olivettifaces, batch_size=40,n_train_batches=320/40=8, paticence=800
    patience = 800
    patience_increase = 2  
        ...
        for minibatch_index in xrange(n_train_batches):
            ...
            if (iter + 1) % validation_frequency == 0:
                ...
                if this_validation_loss < best_validation_loss:
                    ...
                    best_iter = iter
                    save_params(layer0.params,layer1.params,layer2.params,layer3.params)#save parameters
